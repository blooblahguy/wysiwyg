...
(function ($) {
	$.fn.atlas = function(custom_options) {
		 ...
		 */
		private.OnKey = function() {

		}

		/**
		 * @private @function Refresh
		 * Handles refresh on all editor elements
		 */
		...
		atlas.addButton("atlas_separator")
		atlas.addButton("atlas_newtoolbar")
		atlas.addButton("bold", {
			icon: 'bold'
			, keybind: 'ctrl+b'
			, command: 'bold'
		})
		atlas.addButton("italic", {
			icon: 'italic'
			, keybind: 'ctrl+i'
			, command: 'italic'
		})
		atlas.addButton("underline", {
			icon: 'underline'
			, keybind: 'ctrl+u'
			, command: 'underline'
		})
		atlas.addButton("strikethrough", {
			icon: 'strike'
			, keybind: 'ctrl+alt+s'
			, command: 'strikeThrough'
		})
		atlas.addButton("quote", {
			icon: 'quote-left'
			, label: 'Quoteblock'
			, command: function(atlas, command, commandValue) {
				if (atlas.isQuote()) {
		...
		atlas.addButton("clearformatting", {
			icon: 'eraser'
			, label: 'Clear Formatting'
			, command: function(atlas, command, commandValue) {
				document.execCommand("removeFormat", false); // this needs to run twice for some reason
				command = 'removeFormat'
			}
		})
		atlas.addButton("numberlist", {
			icon: 'list-numbered'
			, label: 'Ordered List'
			, command: 'insertorderedlist'
		})
		atlas.addButton("bulletlist", {
			icon: 'list-bullet'
			, label: 'Unordered List'
			, command: 'insertunorderedlist'
		})
		atlas.addButton("leftindent", {
			icon: 'indent-left'
			, label: 'Indent Left'
			, command: function(command, commandValue) {
				if (atlas.isList() || atlas.isQuote()) {
					command = "outdent"
				} else if (atlas.isAlignedCenter() || atlas.isAlignedRight) {
					command = "justifyLeft"
				} else {
					var caret_pos = atlas.getCaretPosition()
					atlas.selectRange(caret_pos - 1, caret_pos)
					if (atlas.getSelectionValue() == "\t") {
						command = "delete"
					} else {
						atlas.selectRange(caret_pos, caret_pos)
					}
				}
		...
		atlas.addButton("rightindent", {
			...
			, command: function(command, commandValue) {
				if (atlas.isList()) {
					...
					// not supported by ie, replace at some point
					command = "insertHTML"
					commandValue = '<span style="white-space:pre">	</span>';
				}
				return [command, commandValue]
			}
			, keybind: 'tab'
		})
		atlas.addButton("alignleft", {
			icon: 'align-left'
			, label: 'Align Left'
			, command: 'justifyLeft'
		})
		atlas.addButton("aligncenter", {
			icon: 'align-center'
			, label: 'Align Center'
			, command: 'justifyCenter'
		})
		atlas.addButton("alignright", {
			icon: 'align-right'
			, label: 'Align Right'
			, command: 'justifyRight'
		})
		atlas.addButton("paragraph", {
			label: 'Paragraph'
			, command: function(atlas, command, commandValue) {
				command = "formatBlock"
				commandValue = "p"
		...
		atlas.addButton("hr", {
			label: 'Insert Horizontal Line Break'
			, icon: ''
			, command: function(atlas, command, commandValue) {
				command = "insertHTML"
				commandValue = '<hr>';
		...
		atlas.addButton("html", {
			icon: 'code'
			, label: 'View HTML'
			, command: function(atlas, command, commandValue) {
				atlas.states.view_html = atlas.states.view_html ? false : true

				if (atlas.states.view_html) {
					$(".atlas_raw_html").show()
				} else {
					$(".atlas_raw_html").hide()
				}
		...
		atlas.addDropdown("textcolor", {
			icon: 'droplet'
			, label: 'Text Color'
		})
		atlas.addDropdown("formatting", {
			icon: 'paragraph'
			, label: 'Text Formatting'
			, html: function() {
				var html = ''
				html += '<p data-cmd="paragraph">Paragraph</p>'
				html += '<h4 data-cmd="h4">Heading 4</h4>'
				html += '<h3 data-cmd="h3">Heading 3</h3>'
				html += '<h2 data-cmd="h2">Heading 2</h2>'
				html += '<h1 data-cmd="h1">Heading 1</h1>'
				return $(html)
			}
			, refresh: function(atlas, btn) {
				if (atlas.isHeader()) {
		...
		atlas.addDropdown("alignment", {
			icon: 'align-left-dropdown'
			...
			, refresh: function(atlas, btn) {
				if (atlas.isAlignedCenter() || atlas.isAlignedRight()) {
					btn.addClass("active")
				} else {
					btn.removeClass("active")
				}
		...
		atlas.addDropdown("image", {
			icon: 'picture'
			, label: 'Insert Image'
		})
		atlas.addDropdown("link", {
			icon: 'link'
			, label: 'Insert/Edit Link'
		})
		atlas.addDropdown("table", {
			icon: 'table'
			, label: 'Insert Table'
		})
		atlas.addDropdown("emoticons", {
			icon: 'smile'
			, label: 'Emoticons'
		})
		...
		private.init = function() {
			...
			atlas.parent.on("mousedown mouseup click", private.buttonHandler)  // captures clicks on the wrapper, determines what to do
		}
		...
		private.hookKeybinds = function() {
			atlas.html_viewer.on("keydown keyup cut paste", function(ev) {
				var html = $(this).val()
				atlas.editor.html(html)
			})

			atlas.editor.on("keydown keyup", function(ev) {
